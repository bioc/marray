...
the same slide \citep{Dudoitetal02}. Although there is no differential
...
\citep{Cleveland79,Norm,NormNAR}. Local regression is a {\it smoothing}
...
\citep{Norm,NormNAR}. For a collection of numbers $x_1, \ldots, x_n$,
...
\end{description}
 ...
 \citep{NormNAR}. The {\tt maNormMain} function returns objects of class
...
geometric mean of the MADs across groups \citep{NormNAR}. This allows
...
mean of MAD across slides \citep{Norm,NormNAR} 
