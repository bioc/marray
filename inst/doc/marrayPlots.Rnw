...
in {\tt marray} package, which is a packages for diagnostic plots and
normalization of cDNA microarray data.  %%%A shorter guide to the
package is given in {\tt marrayPlotsShort} and {\tt marray}.  This
package provides functions for diagnostic plots of microarray spot
statistics, such as boxplots, scatter--plots, and spatial color
...
which can lead to biased inference concerning gene expression.
...
A quick start guide on this package are given in the {\tt marray}
document of the {\tt inst/doc} directory.  \\
...
tmp<-image(swirl[,3], x="maGb", subset=TRUE, col=Gcol,contours=FALSE, bar=FALSE) 
@
<<maImageRb,fig=TRUE,prefix=FALSE,echo=TRUE,include=FALSE>>=
tmp<-image(swirl[,3], x="maRb", subset=TRUE, col=Rcol,
...
\begin{verbatim}
image(swirl[,3], x="maGb") 
image(swirl[,3], x="maRb") 
\end{verbatim}
%%@
...
tmp<-image(swirl[,3], x="maM", bar=FALSE, main="Swirl array 93: image of pre--normalization M") 
@

<<maImageMraw2,fig=TRUE,prefix=FALSE,echo=TRUE,include=FALSE>>= 
tmp<-image(swirl[,3], x="maM", subset=maTop(maM(swirl[,3]), h=0.10, l=0.10), col=RGcol, contours=FALSE, bar=FALSE,main="Swirl array 93: image of pre--normalization M for \% 10 tails") 
@
...
marrayTwo} (see details in {\tt ? boxplot}). The function {\tt
boxplot} has three main arguments \begin{description} \item {{\tt
...
boxplot(swirl[,3], x="maPrintTip", y="maM", main="Swirl array 93: pre--normalization") 
@
...
boxplot(swirl, y="maM", main="Swirl arrays: pre--normalization") 
@
...
boxplot(swirl.norm[,3], x="maPrintTip", y="maM",
	main="Swirl array 93: post--normalization") 
@

<<maBoxplot2post,fig=TRUE,prefix=FALSE,echo=TRUE,include=FALSE>>=
boxplot(swirl.norm, y="maM", col="green", main="Swirl arrays: post--normalization") 
@
...
The function {\tt plot} produces {\it scatter--plots} of microarray
...
marrayRaw} or  {\tt marrayNorm}.  \item {{\tt x}:}
...
plot(swirl[,3], x="maA", y="maM", z="maPrintTip",
		      lines.func,
		      text.func=maText(),
		      legend.func,
...
plot(swirl.norm[,3], x="maA", y="maM", z="maPrintTip",
		      lines.func,
		      text.func=maText(),
		      legend.func,
...
that an intensity or $A$--dependent normalization method is preferable
to a global one (e.g. median normalization). Also, the lowess fits vary
among print--tip--groups, again revealing the existence of spatial dye
...
More details of this can be found in the package {\tt arrayQuality}.
