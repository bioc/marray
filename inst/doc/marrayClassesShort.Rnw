...
which should have equal abundance in the two target samples, such as housekeeping genes).  
\item
...
%%% As described in greater details in Section \ref{sInput}, the input
functions in {\tt marrayInput} were used to generate microarray objects
{\tt swirl.gnames}, {\tt swirl.samples}, {\tt swirl.layout}, and {\tt
swirl} for the Swirl experiment (the first two are of class {\tt
marrayInfo}, and the last two of class {\tt marrayLayout} and {\tt
marrayRaw}, respectively).   
...
To see all methods available for a particular class, e.g., {\tt
marrayLayout}, or just the print methods 
...
such as {\tt swirl}, can be obtained by {\tt print(swirl)} or simply
{\tt swirl}. 
...
{\tt maLayout(swirl)}.\\ 
 ...
 directly on objects of class {\tt marrayRaw}, {\tt marrayNorm}, {\tt
 marraySpots}, and {\tt marrayTwo}. The commands below may be used to
 display the number of spots on the array, the dimensions of the grid
 matrix, and the print--tip--group coordinates. 
...
{\tt Biobase} and {\tt convert} package)
...
coordinates and gene names for objects of class {\tt marrayRaw}, {\tt
marrayNorm}.  
